  ...
  render() {
    ...
    if (commentAllowed === true && rating.rating > 0 && rating.rating <= 2) {
      if (this.state.comment.length < 50) {
        ratingTooltip = (
          <Tooltip title="You have give a score lower than 3 which requires you to provide some further input that is longer than 50 characters in length">
...
class DefaultView extends Component {
  ...
  welcomeScreen() {
    ...
    const defaultWelcomeMessage = (
      <Typography gutterBottom>
        Thank you for taking the time to assess{" "}
        {assessment.selfAssessment === true
          ? "yourself"
          : api.getUserFullName(assessment.delegate)}
        . This assessment should take approximately 5 - 7 minutes to complete.
        <br />
        You will be asked to provide a rating against a series of behaviours
        that are used to measure how{" "}
        {isPLC === true
          ? `well the ${survey.leadershipBrand.title} are displayed:`
          : ` we live the organisation's leadership brand:`}
      </Typography>
    );
        ...
        {assessment.complete === false && (
          <Fragment>
            ...
            <Button
              ...
              style={{ marginRight: "4px", color: palette.success.main }}
            >
              <Icon>check_outline</Icon>&nbsp;Finish
            </Button>
          </Fragment>
        )}
      </Paper>
  ...
  loadCustomQualityComment() {
    ...
    const { step, assessment, newBehaviourText, qualityCustomComment } =
      this.state;
  ...
  `;
    ...
    api
      .graphqlQuery(contentQuery, { slug: commentSlug })
      .then((result) => {
        ...
        api.log(
          `Results from fetching custom comment`,
          { data, errors },
          "debug"
        );
        ...
        api
          .graphqlQuery(contentQuery, { slug: adminCommentSlug })
          .then((adminCommentResult) => {
          ...
          .catch((adminCommentGetError) => {
            api.log(
              `Could not load the admin action ${adminCommentGetError.message}`
            );
            that.setState({ qualityCustomComment, qualityAction });
          });
      })
    ...
    switch (assessment.survey.surveyType) {
      ...
      case "team180": {
        ...
        const staticContentProps = {
          ...
          showEditIcon: false,
          helpTopics: [
            `mores-assessment-help-personalized-comment-${api.utils.slugify(
              quality.title
            )}`,
          ],
  ...
  nextStep() {
    let maxSteps =
      this.props.assessment.survey.leadershipBrand.qualities.length + 2;
  ...
  render() {
    ...
    let buttonColor =
      step > 0 && isCurrentStepValid === false && isThankYou === false
        ? "default"
        : "secondary";
