...
function testIntelligentCaching() {
    ...
    function testCacheHit() {
        ...
        const isCacheHit = cachedResult && (Date.now() - cachedResult.timestamp) < 300000;
    ...
    function testCacheInvalidation() {
        const cache = new Map();
        const queryKey = JSON.stringify({ query: mockGraphQLQuery, variables: mockGraphQLVariables });
        cache.set(queryKey, { data: mockFormData, timestamp: Date.now() - 600000 });
        const cachedResult = cache.get(queryKey);
        const isStale = cachedResult && (Date.now() - cachedResult.timestamp) > 300000;
        if (isStale) {
...
//# sourceMappingURL=phase2DataManagementTests.js.map