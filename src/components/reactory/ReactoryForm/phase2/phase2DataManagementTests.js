...
function testDataPrefetching() {
  ...
  function testPrefetchTiming() {
    ...
    setTimeout(() => {
      ...
      console.assert(true, 'Prefetch timing test passed');
...
function testOfflineSupport() {
  ...
  function testSyncQueue() {
    ...
    // Process sync queue - all items should be pending since they were just created
...
function testDataCompression() {
  ...
  function testDecompressionAccuracy() {
    ...
    // For testing, we'll just verify the basic structure is preserved
    const isAccurate = decompressed.includes('John') && decompressed.includes('john');
